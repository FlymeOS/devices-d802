...
DIGICL_VER=`getprop ro.build.version.sdk`.2.0
...
do
    ...
    e)
        ...
        if [ ! -n "$EXTERNAL_ADD_STORAGE" ]; then
            ...
            /system/bin/sh /system/bin/am broadcast -a com.lge.android.digicl.intent.COPYLOGS_FAILED --ei error_code -3
...
if [ ! -e "$PATH_TARGET/../.nomedia" ]; then
    ...
    /system/bin/sh /system/bin/am broadcast -a com.lge.android.digicl.intent.COPYLOGS_FAILED --ei error_code -2
...
dumpsys dropbox --print > $PATH_DROPBOX_TMP/dropbox_print.txt
...
dumpsys dropbox SYSTEM_TOMBSTONE --print > $PATH_DROPBOX_TMP/SYSTEM_TOMBSTONE.txt

dumpsys dropbox system_server_anr --print > $PATH_DROPBOX_TMP/system_server_anr.txt
dumpsys dropbox system_app_anr --print > $PATH_DROPBOX_TMP/system_app_anr.txt
dumpsys dropbox data_app_anr --print > $PATH_DROPBOX_TMP/data_app_anr.txt

dumpsys dropbox system_server_crash --print > $PATH_DROPBOX_TMP/system_server_crash.txt
dumpsys dropbox system_app_crash --print >  $PATH_DROPBOX_TMP/system_app_crash.txt
dumpsys dropbox data_app_crash --print > $PATH_DROPBOX_TMP/data_app_crash.txt
...
dumpsys dropbox --file > $PATH_DROPBOX_TMP/dropbox_files.txt
echo cp -rv $PATH_DROPBOX_TMP $PATH_TARGET/
cp -rv $PATH_DROPBOX_TMP $PATH_TARGET/
echo rm -rvf $PATH_DROPBOX_TMP
rm -rvf $PATH_DROPBOX_TMP
...
/system/bin/sh /system/bin/am broadcast -a com.lge.android.digicl.intent.COPYLOGS_COMPLETED --es path $PATH_SEND
